...
export async function getMyPlaces(): Promise<MyPlaces> {
  ...
  const { data, error } = await supabase
    .from("user_places")
    .select("place_id, state, updated_at, note, avoid_reason")
    .eq("user_id", userData.user.id)
    .order("updated_at", { ascending: false });
  if (error) throw error;

  const rows = (data ?? []).filter((row: any) => typeof row.place_id === "string" && isPlaceState(row.state));
  const pins = await getPlacePins(rows.map((row: any) => row.place_id));
  for (const row of rows as any[]) {
    const pin = pins[row.place_id];
    grouped[row.state as PlaceState].push({
      placeId: row.place_id,
      name: pin?.name ?? "Saved place",
      state: row.state,
      updatedAt: row.updated_at,
      placeRating: pin?.rating ?? null,
      note: row.note ?? null,
